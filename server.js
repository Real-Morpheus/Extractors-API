...
async function raceProxies(url) {
  const strategies = [
    ...
    // 5. Direct — on Render the server IP may not be blocked
...
async function hubcloudExtract(url) {
  ...
  for (const p of jsP) { const v = reGet(html1, p); if (v && v.startsWith('http')) { redirectUrl = v; break; } }
...
async function gdflixExtract(url) {
  ...
  let html;
  try {
    ({ text: html } = await scrapeHtml(url));
  ...
  // Pixeldrain
  if (pixelHref && pixelHref.includes('pixeldrain')) {
...
async function gofileExtract(id) {
  try {
    ...
    const ch = cd.data && cd.data.children;
...
async function filepressExtract(url) {
  ...
  if (!fm) return [];
  const fid = fm[1];
  const base = /^(https:\/\/[^/]+)/.exec(pl) ? /^(https:\/\/[^/]+)/.exec(pl)[1] : 'https://new1.filepress.cloud';
  try {
    ...
    const alts = (fi.data && fi.data.alternativeSource) ? fi.data.alternativeSource : [];
    ...
    if (!s2d.status || !s2d.data || !s2d.data.downloadId) throw new Error('no downloadId');
...
async function handleBypass(req, res) {
  ...
  if (!target) return res.status(400).json({ error: 'Missing url', usage: 'GET /bypass?url=<encoded>' });
...
app.use((req, res) => {
  res.status(404).json({
    error: 'Not found',
    routes: ['GET /', 'GET /health', 'GET|POST /bypass?url=', 'GET /extract/{vcloud,hubcloud,gdflix,gdirect,filepress,gofile}?url='],
  });
});
...
app.listen(PORT, () => {
  console.log(`Stream Bypass API running on port ${PORT}`);
});
